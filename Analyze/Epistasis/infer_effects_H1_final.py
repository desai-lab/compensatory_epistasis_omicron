...
for f in range(num_folds):
	...
	if ep_type == 'stat':
		...
		fold = f
	with open('model_coefs/temp_ACE2_CV_rsquared_'+str(fold)+'_'+ep_type+'.csv','r') as readfile:
...
with open('model_coefs/ACE2_CV_rsquared_'+ep_type+'.csv','w') as writefile:
...
mutations_H1 = [str(x) for x in range(1,16)]

with open('cleaned_Kds_RBD_ACE2.tsv','r') as readfile:
    kd_reader = csv.reader(readfile,delimiter="\t")
...
for order in range(1,optimal_H1_order+1):
    ...
    with open('model_coefs/ACE2_'+str(order)+'order_'+ep_type+'.txt','w') as writefile:
